...
try {
    if ($_SESSION['role'] === 'admin') {
        $stmt = $pdo->query("SELECT id, student_id, first_name, last_name, grade FROM children WHERE status = 'active' ORDER BY first_name, last_name");
    } else {
        // Teachers see only their assigned children
        $stmt = $pdo->prepare("SELECT c.id, c.student_id, c.first_name, c.last_name, c.grade 
                              FROM children c 
                        ...
                        <select id="child_id" name="child_id" class="form-control" required>
                            ...
                            <?php foreach ($children as $child): ?>
                                <option value="<?php echo $child['id']; ?>" <?php echo (isset($_POST['child_id']) && $_POST['child_id'] == $child['id']) ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($child['first_name'] . ' ' . $child['last_name'] . ' (' . $child['student_id'] . ') - ' . $child['grade']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
