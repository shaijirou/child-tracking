...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $required_fields = ['first_name', 'last_name', 'date_of_birth', 'student_id', 'parent_id'];
        ...
        $stmt = $pdo->prepare("SELECT id FROM children WHERE student_id = ? AND id != ?");
        $stmt->execute([$_POST['student_id'], $child_id]);
        if ($stmt->fetch()) {
        ...
        if ($child_id) {
            ...
            $stmt = $pdo->prepare("UPDATE children SET 
                                  ...
                                  student_id = ?, grade = ?, school = ?, parent_id = ?, 
                                  device_id = ?, status = ?, medical_info = ?, updated_at = NOW()
                                  WHERE id = ?");
            $stmt->execute([
                $_POST['first_name'], $_POST['last_name'], $_POST['date_of_birth'],
                $_POST['student_id'], $_POST['grade'], $_POST['school'], $_POST['parent_id'],
                $_POST['device_id'], $_POST['status'], $_POST['medical_info'], $child_id
            ]);
        } else {
            // Insert new child
            $stmt = $pdo->prepare("INSERT INTO children 
                                  (first_name, last_name, date_of_birth, student_id, grade, school, 
                                   parent_id, device_id, status, medical_info, created_at, updated_at) 
                                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
            $stmt->execute([
                $_POST['first_name'], $_POST['last_name'], $_POST['date_of_birth'],
                $_POST['student_id'], $_POST['grade'], $_POST['school'], $_POST['parent_id'],
                $_POST['device_id'], $_POST['status'], $_POST['medical_info']
            ]);
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="student_id" name="student_id" 
                                           value="<?php echo htmlspecialchars($child['student_id']); ?>" 
                                           class="form-control" required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="device_id" class="form-label">Device ID (IMEI)</label>
                                    <input type="text" id="device_id" name="device_id" 
                                           value="<?php echo htmlspecialchars($child['device_id']); ?>" 
...
function testDevice() {
    ...
    fetch(`api/test_device.php?device_id=${deviceId}`)
...
document.addEventListener('DOMContentLoaded', function() {
    const deviceId = '<?php echo $child['device_id']; ?>';
    if (deviceId) {
...
function checkDeviceStatus(deviceId) {
    fetch(`api/check_device_status.php?device_id=${deviceId}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
